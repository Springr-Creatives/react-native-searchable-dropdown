...
export default class SearchableDropDown extends Component {
  ...
  searchedItems = searchedText => {
    textType = searchedText.charAt(0)
    let setSort = this.props.setSort;
    if (!setSort && typeof setSort !== 'function') {
      setSort = (item, searchedText) => {
        if (['1', '2', '3', '4', '5', '6', '7', '8', '9', '0'].includes(searchedText.charAt(0))) {
          console.log("item.serialNumber==>", item.serialNumber);
          return item.serialNumber?.toString().toLowerCase().indexOf(searchedText.toLowerCase()) > -1
        } else {
          console.log("item.customerName==>", item.customerName);
          return item.customerName.toLowerCase().indexOf(searchedText.toLowerCase()) > -1
        }
      };
    }
        ...
        this.props.selectedItems.find(sitem => sitem.id === item.id)
          ...
          <TouchableOpacity style={{ ...this.props.itemStyle, flex: 1, flexDirection: 'row' }}>
            <View style={{ flex: 0.9, flexDirection: 'row', alignItems: 'flex-start' }}>
              {item.serialNumber != '' ? <Text>{item.serialNumber}</Text> :
        ...
        >
          {
            this.props.selectedItems && this.props.selectedItems.length > 0 && this.props.selectedItems.find(x => x.id === item.id)
              ?
              <Text style={{ ...this.props.itemTextStyle }}>{['1', '2', '3', '4', '5', '6', '7', '8', '9', '0'].includes(textType) ? item.serialNumber : item.customerName}</Text>
              :
              <Text style={{ ...this.props.itemTextStyle }}>{['1', '2', '3', '4', '5', '6', '7', '8', '9', '0'].includes(textType) ? item.serialNumber : item.customerName}</Text>
          }
        </TouchableOpacity>
  ...
  renderTextInput = () => {
    ...
    const oldSupport = [
      ...
      {
        key: 'value',
        val: this.state.item ? this.state.item.serialNumber != '' ? this.state.item.serialNumber : this.state.item.customerName : ''
      },
      {
        key: 'style',
        ...
        {items.map((item, index) => {
          return (
            <View key={index} style={{
              width: (item.serialNumber != '' ? item.serialNumber.length * 8 : item.customerName.length * 8) + 60,
              ...
              {item.serialNumber != '' ? <Text style={{ color: '#555' }}>{item.serialNumber}</Text> :
                <Text style={{ color: '#555' }}>{item.customerName}</Text>}
              <TouchableOpacity onPress={() => setTimeout(() => { this.props.onRemoveItem(item, index) }, 0)} style={{ backgroundColor: '#f16d6b', alignItems: 'center', justifyContent: 'center', width: 25, height: 25, borderRadius: 100, marginLeft: 10 }}>
                <Text>X</Text>
              </TouchableOpacity>
            </View>
